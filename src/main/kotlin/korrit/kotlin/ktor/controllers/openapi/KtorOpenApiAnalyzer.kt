...
import korrit.kotlin.openapi.Header
import korrit.kotlin.openapi.MediaType
import korrit.kotlin.openapi.OpenApi
import korrit.kotlin.openapi.Operation
import korrit.kotlin.openapi.Parameter
import korrit.kotlin.openapi.Path
import korrit.kotlin.openapi.Property
import korrit.kotlin.openapi.RequestBody
import korrit.kotlin.openapi.Response
import korrit.kotlin.openapi.Schema
...
open class KtorOpenApiAnalyzer(
        ...
        val defaultHeaders: List<HttpHeader> = emptyList(),
        val defaultContentType: ContentType = ContentType.Application.Json,
        val defaultErrorType: KClass<*>
) {

    private val routes = mutableListOf<Path>()
    ...
    protected fun analyzeRoute(route: Route) {
    ...
    protected fun analyzeMethod(route: Route): String {
        return (route.selector as HttpMethodRouteSelector).method.value.toLowerCase()
    }

    protected fun analyzeInput(route: Route): Triple<Boolean, List<Parameter>?, RequestBody?> {
        ...
        val parameters = input::class.memberProperties
                ...
                .mapNotNull { prop ->
                    ...
                    when (delegate) {
                        null -> null
                        is PathParamDelegate<*> -> Parameter(delegate.name, "path", delegate.required, isDeprecated, null, toSchema(prop.returnType, default = if (delegate.required) null else delegate.default))
                        is QueryParamDelegate<*> -> Parameter(delegate.name, "query", delegate.required, isDeprecated, null, toSchema(prop.returnType, default = if (delegate.required) null else delegate.default))
                        is HeaderParamDelegate<*> -> Parameter(delegate.name, "header", delegate.required, isDeprecated, null, toSchema(prop.returnType, default = if (delegate.required) null else delegate.default))
                        else -> throw Exception("Unknown delegate in class ${input::class.qualifiedName}")
                    }
                }
    ...
    private fun analyzeResponses(route: Route, path: String): List<Response> {
        ...
        return responseTypes.map {
            ...
            val responseHeaders = defaultHeaders + (it.headers ?: emptyList())
    ...
    protected fun toSchema(contentType: ContentType, type: KType): Schema {
    ...
    protected fun toSchema(type: KType, deprecated: Boolean = false, default: Any? = null): Schema {
    ...
    protected fun objectToSchema(classifier: KClass<*>, deprecated: Boolean, nullable: Boolean): Schema {
        ...
        val constructorParams = classifier.primaryConstructor?.parameters ?: throw AnalysisException("Cannot transform type ${classifier.simpleName} to OpenApi Schema Object: missing primary constructor")
        val required = constructorParams.asSequence()
                .filter { param ->
                    properties.find { it.name == param.name } ?: throw AnalysisException("Cannot transform type ${classifier.simpleName} to OpenApi Schema Object: primary constructor param is not a public property: ${param.name}")
                    !param.isOptional
                }
                .map {
                    it.name!!
    ...
    protected fun enumToSchema(classifier: KClass<*>, deprecated: Boolean, nullable: Boolean, default: Any? = null): Schema {
    ...
    protected fun mapToSchema(type: KType, classifier: KClass<*>, deprecated: Boolean, nullable: Boolean): Schema {
    ...
    protected fun setToSchema(type: KType, deprecated: Boolean, nullable: Boolean): Schema {
    ...
    protected fun listToSchema(type: KType, deprecated: Boolean, nullable: Boolean): Schema {
    ...
    protected fun arrayToSchema(type: KType, classifier: KClass<*>, deprecated: Boolean, nullable: Boolean): Schema {
    ...
    protected fun findSuperType(haystack: KType, needle: KClass<*>): KType? {
    ...
    protected fun findSuperType(haystack: KClass<*>, needle: KClass<*>): KType? {
    ...
    protected fun findMapValueType(map: KType): KType? {
